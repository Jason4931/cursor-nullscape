...
export function setup(host, hardMode) {
  ...
  function instrumentCollected() {
    state.phase++;

    if (state.phase >= (hardMode ? 3 : 4)) {
      state.phase = hardMode ? 3 : 4;
  ...
  function update(dt) {
    ...
    state.layerspeed += dt;
    if (
      state.layerspeed >= 1 / Math.pow(state.phase + 1, 2) ||
      state.mode == "agro"
    ) {
      state.layer++;
    ...
    if (state.mode === "idle") {
      if (state.soundState != state.phase && state.sound) {
        state.sound();
        state.sound = null;
      }
      state.soundState = state.phase;
      if (!state.sound)
        state.sound = playSound(
          `./ASSET/Sound/Enemies/Cadence/Cad_lv${state.phase + (hardMode ? 2 : 1)}.wav`,
          undefined,
          undefined,
          undefined,
          () => {
  ...
  function drawChain(ctx, x1, y1, x2, y2) {
    ...
    grad.addColorStop(0, "#270000");
    grad.addColorStop(0.5, "#5a0000");
    grad.addColorStop(1, "#270000");
    ...
    if (!uldm) {
      ...
      for (let d = 0; d < dist; d += twistSpacing) {
        ...
        twistGrad.addColorStop(0, "rgba(255,0,0,0)");
        twistGrad.addColorStop(0.5, "rgba(255,0,0,0.25)");
        twistGrad.addColorStop(1, "rgba(255,0,0,0)");
  ...
  function draw(ctx) {
    ...
    if (!uldm) {
      ...
      grad.addColorStop(0, "rgba(128,0,0,1)");
      grad.addColorStop(0.6, "rgba(128,0,0,1)");
    ...
    for (const it of state.instruments) {
      ...
      if (it.pickedUp) {
        ...
        if (!uldm) {
          ...
          g.addColorStop(0, "rgba(128,0,0,1)");
          g.addColorStop(0.4, "rgba(128,0,0,1)");
          g.addColorStop(1, "rgba(128,0,0,0)");
          ctx.fillStyle = g;
          ctx.beginPath();
          ctx.arc(ix, iy, 50, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.filter = "sepia(100%) saturate(3000%) hue-rotate(-5deg)";
