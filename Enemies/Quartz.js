...
export function setup(host) {
  ...
  for (const ring of state.glyphRings) {
    ...
    octx.fillStyle = rainbow(((ring.radius - 40) / 20) * 0.5);
  ...
  function rainbow(offset = 0, alpha = 1) {
    ...
    const r = Math.sin(t + offset) * 127 + 128;
    const g = Math.sin(t + offset + 2) * 127 + 128;
    const b = Math.sin(t + offset + 4) * 127 + 128;
  ...
  function draw(ctx) {
    ...
    ESP(state.x, state.y, 400, "quartz");
